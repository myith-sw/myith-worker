...
**Core가 알려줘야 할 것 (값 확정 요청):**
- 수신(작업 큐, 경쟁 소비) — Core가 아래를 발행하는 **큐명(또는 exchange+routing key)**:
  - `AiEnhancementRequested` (우선순위 1 필수)
  - `RoadmapGenerationRequested`, `JobProfileBuildRequested`
- 발신(fanout, 브로드캐스트) — Core가 수신하는 **fanout 익스체인지명**:
  - `AiEnhancementCompleted` · `RoadmapGenerationProgress` · `CompetencyExtracted` · `JobProfileBuilt`
- DLQ 이름·정책을 Worker가 선언해도 되는지(아니면 Core 소유인지).
- 큐/익스체인지의 **선언 소유권**: 누가 declare하고 durable/auto-delete 설정은 무엇인지.

**Worker 쪽 계약 (구현됨/예정):**
- 봉투는 D-1 고정: `{eventId, eventType, version, traceId, occurredAt, payload}`. 발신 `eventId`는
  (eventType+대상ID)로 결정론적(재발행 동일, W2 C-3). 수신 멱등은 `worker_processed_event`(DB 고유제약).
- `AiEnhancementCompleted` payload: `requestId`(순수 UUID, 접두어 없이 그대로)·`roadmapId` 필수,

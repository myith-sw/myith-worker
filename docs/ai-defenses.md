...
| **LLM 서킷브레이커 + 실호출 검증** | 리뷰가 "LLM 경로에 브레이커가 아예 없다(C-5 위반)"를 지적. 선불 $5라 402·429가 실제 시나리오. | 외부 LLM·GitHub·Vision 호출에 브레이커. | **잘못된 키로 api.anthropic.com에 실제 401**을 내서 역량추출 []·STAR FAILED·문구 층1 폴백을 실측(2026-07-29) | [provider.py](../app/llm/provider.py) |
...
> **OCR 주의:** 위 브레이커 목록에 OCR이 빠진 것은 의도다 — OCR은 데모 `.env`에 자격증명 미설정이라
> 경로 자체가 안 탄다(**데모 범위 밖**). 자격증명이 붙는 운영에선 OCR에도 브레이커·타임아웃이 필요하다.
